...
This template includes a WebSocket server implementation that allows bidirectional communication between the server and clients. The WebSocket functionality enables real-time tool execution on the client side.

### WebSocket Server Features

- **Connection Management**: Handles client connections with unique IDs
- **Tool Registration**: Clients can register their available tools during connection
- **Bidirectional Communication**: Server can request tool execution from clients
- **Asynchronous Response Handling**: Uses futures to handle tool execution responses

### Server Implementation

The WebSocket server is implemented in two main files:

1. `src/react_agent/web/server.py`:
   - FastAPI WebSocket endpoint at `/ws`
   - Handles connection lifecycle
   - Processes incoming messages and tool responses
   - Supports test tool execution on connection

2. `src/react_agent/web/connection.py`:
...
You can test the WebSocket functionality using the provided test client in `client/src/test_websocket.js`. Here's how to run the test:

1. Start the FastAPI server:
```bash
uvicorn src.react_agent.web.server:app --reload
```
...
Here's a basic example of implementing a client with tool support:
...
const tools = {
    test_tool: {
        name: "test_tool",
        description: "A test tool that returns system information",
        parameters: {}
    }
};
...
ws.onmessage = async (event) => {
    ...
    if (message.type === 'tool_call') {
        const { event_id, data } = message;
        try {
            const result = await toolImplementations[data.name]();
...
1. **Tool Registration (Client → Server)**:
```json
{
    "tools": {
        "tool_name": {
